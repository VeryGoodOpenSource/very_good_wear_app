    ...
    Directory(path.join(_targetPath, 'my_app'))
      ...
      try {
        ...
        file = await file.writeAsString(
          contents
              .replaceAll(
                  'very_good_wearos_app', '{{project_name.snakeCase()}}')
              .replaceAll(
                  'very-good-wearos-app', '{{project_name.paramCase()}}')
              .replaceAll('A new Flutter WearOS project..', '{{{description}}}')
              .replaceAll(
                  'Very Good WearOS App', '{{project_name.titleCase()}}')
              .replaceApplicationId(file.path),
        );

        final fileSegments = file.path.split('/').sublist(2);
        if (fileSegments.contains('very_good_wearos_app')) {
          final newPathSegment = fileSegments.join('/').replaceAll(
                'very_good_wearos_app',
